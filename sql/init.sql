...
-- 「我的资源」门户侧专属菜单 + 权限串
--   admin 端 /biz/resource/* 用的是 resource:list,但学生角色无此权限(见 role=2 的 sys_role_menu 段)
--   所以新增独立菜单项 + 独立权限串 resource:my:list,只为门户路由守卫消费
INSERT INTO `sys_menu`(`id`, `parent_id`, `menu_name`, `menu_type`, `path`, `component`, `icon`, `permission`, `sort`, `visible`, `status`) VALUES
(950, 753, '我的资源',     3, NULL, NULL, NULL, 'resource:my:list',  7, 1, 1);

-- 学生(role=4)获得 resource:my:list 权限
INSERT INTO `sys_role_menu`(`id`, `role_id`, `menu_id`)
SELECT ROW_NUMBER() OVER (ORDER BY id) + 9000, 4, id FROM sys_menu
WHERE id = 950
  AND id NOT IN (SELECT menu_id FROM sys_role_menu WHERE role_id = 4);
...
-- 真实教师授课关系：张教授（1001）任数据结构、操作系统；
-- 李副教授（1002）任软件工程、Web应用开发；
-- 让「授课管理」页面有真实数据可看，也方便"按课程/班级"分析
INSERT INTO `course_teacher`(`id`, `course_id`, `teacher_id`, `role`, `status`) VALUES
(101, 2, 1001, '主讲', 1),  -- 张教授 → 数据结构
(102, 3, 1001, '主讲', 1),  -- 张教授 → 操作系统
(103, 5, 1002, '主讲', 1),  -- 李副教授 → 软件工程
(104, 6, 1002, '主讲', 1);  -- 李副教授 → Web应用开发
